import be.angelcorp.libs.celest.universe.modules
...
implicit val universe = new Universe {
  ...
  def frames = ??? // ??? is a method that throws NotImplementedError
  // Make TT, TAI, UTC, and UT1 all the same timeline (so effectively there is no TAI/UTC/UT1)
  val TT     = new be.angelcorp.libs.celest.time.timeStandard.TT()
  def TAI    = TT
  def UTC    = TT
  def UT1    = TT
