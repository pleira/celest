...
class JplDeAsciiParser(implicit universe: Universe) extends Parser {
	...
	def Group105 = rule {
		(str("GROUP") ~ WhiteSpace ~ str("1050") ~ WhiteSpace ~ oneOrMore(Integer ~ WhiteSpace)) ~~>
			(l => {
				JplDeHeaderGroup105(l.sliding(3).toList.transpose.collect {
	...
	def Keyword = rule {
		(!str("GROUP") ~ oneOrMore("0" - "9" | "a" - "z" | "A" - "Z")) ~> (s => s)
	}
	...
	def Integer = rule(SuppressSubnodes) {
		(optional(Sign) ~ (("1" - "9") ~ Digits | Digit)) ~> (s => s toInt)
	}

	def Float = rule {
		(Exponential | Fractional ~> (s => s toDouble))
	}

	def Exponential = rule(SuppressSubnodes) {
		group(Fractional ~ Exponent) ~> (s => s.replaceAll("[dDe]", "E") toDouble)
	}

	def Fractional = rule(SuppressSubnodes) {
		optional(Sign) ~ ((oneOrMore(Digits) ~ "." ~ optional(zeroOrMore(Digits))) | (zeroOrMore(Digits) ~ "." ~ oneOrMore(Digits)))
	}
	...
	def Exponent = rule {
		((ignoreCase("e") | ignoreCase("d")) ~ optional(Sign) ~ Digits)
	}
	...
	def WhiteSpace = rule(SuppressSubnodes) {
		zeroOrMore(anyOf(" \n\r\t\f"))
	}
