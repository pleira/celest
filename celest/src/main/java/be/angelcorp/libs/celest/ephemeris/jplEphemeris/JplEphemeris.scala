...
import be.angelcorp.libs.celest.time.{TimeRange, Epoch}
...
import be.angelcorp.libs.celest.util.{AlignmentStrategy, PackedAlignment, RichByteBuffer}
...
class DataRecord(val metadata: Metadata, val data: Array[Double] )(implicit universe: Universe) {
  val begin = JulianDate(data(0), universe.TDB) // Beginning time of the record
  val end   = JulianDate(data(1), universe.TDB) // Ending    time of the record
