...
class Spherical(val r: Double,
                ...
                val γ: Double,
                val ψ: Double,
                val frame: Option[frames.BodyCentered] = None) extends Orbit {
  ...
  def toPosVel = {
    ...
    val cosD  = cos(δ);      val sinD  = sin(δ)
    val cosAl = cos(α);      val sinAl = sin(α)
    val cosA  = cos(ψ);      val sinA  = sin(ψ)
    val cosG  = cos(γ);      val sinG  = sin(γ)
...
object Spherical {
  ...
  /**
   * Create a set of [[be.angelcorp.libs.celest.state.Spherical]] from another
   * [[be.angelcorp.libs.celest.state.Orbit]]. Chooses itself what the best way of converting is.
   *
  ...
  /**
   * Convert [[be.angelcorp.libs.celest.state.PosVel]] to [[be.angelcorp.libs.celest.state.Spherical]] elements.
   *
