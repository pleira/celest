...
object TimeUtils {
  ...
  /**
	 ...
	 * When it is inside this interval, use [[be.angelcorp.libs.celest.time.TimeUtils#jday(Int, Int, Int, Int, Int, Double)]].
	 * </p>
	 * 
   ...
   * [[be.angelcorp.libs.celest.time.TimeUtils#gregorian_jd(Int, Int, Int, Int, Int, Double)]]
	 * </p>
	 * 
	 * @param year Year e.g. 1900 .. 2100
	 * @param mon  Month e.g. 1 .. 12
	...
	def rad_hms(rad: Double) = {
		val temp = (rad / 15.0) * (180.0 / Pi); // [([rad]/[deg/hr]) * [deg/rad]] = [hr]
    val hr  = temp.toInt
  ...
  @deprecated
     ...
     */
    def dayFraction( jd: Double ) = (jd + 0.5 - floor( jd )) % 1.0; // JD starts at midday so add 0.5 to align to midnight
