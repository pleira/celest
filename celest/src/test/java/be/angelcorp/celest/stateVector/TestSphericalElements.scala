...
class TestSphericalElements extends FlatSpec with ShouldMatchers {
  ...
  "Spherical" should "convert the Cartesian elements from/to Spherical for geostationairy orbits" in {
    ...
    val s_expected = new Spherical(rGEO, 0, 0, vc, 0, Pi / 2.0, Some(jplEarthFrame))
  ...
  "Spherical" should "convert the Keplerian elements from/to Spherical for the lunar position" in {
    ...
    val k = Keplerian(3.903213584163071E+08, 4.074916709908236E-002, 9.218093894982124E-2,
      4.850831512485626E+00, 4.757761494574442E+00, TrueAnomaly(1.079822859502195E00), Some(jplEarthFrame))
    val s_expected = new Spherical(3.823277207168130E+08, -1.876578287892178E00, -3.178799710706510E-2,
      1.0315839033233294E+03, Pi / 2.0 - 1.535552685601132E00, 1.484255164403220E00, Some(jplEarthFrame))
  ...
  "Spherical" should "return itself when cloned" in {
    val s_expected = new Spherical(3.823277207168130E+08, -1.876578287892178E00, -3.178799710706510E-2,
      1.031461835283901E+03, Pi / 2.0 - 1.535552685601132E00, 1.484255164403220E00, Some(jplEarthFrame))
