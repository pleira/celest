...
class TestKeplerian extends FlatSpec with ShouldMatchers {
  ...
  "Keplerian" should "convert the Cartesian elements from/to Keplerian for 2002 TZ300" in {
    ...
    val k_true = Keplerian(AU.convert(43.69), 0.0, Deg.convert(3.58), Deg.convert(334.67),
      Deg.convert(55.46), TrueAnomaly(Deg.convert(1.25)), Some(sunFrame))

    val pv = PosVel(5.575650800580360E12, 3.406285950357226E12, -0.166516757517559E12,
      -2.339772644074335E3, 3.842463945033625E3, 0.256885464567324E3, Some(sunFrame))
  ...
  it should "convert the Cartesian elements from/to Keplerian for 2000 QF226" in {
    ...
    val k_true = Keplerian(AU.convert(46.49), 0, Deg.convert(2.25), Deg.convert(296.90),
      Deg.convert(41.00), TrueAnomaly(Deg.convert(5.75)), Some(sunFrame))

    val pv = PosVel(6.670590824266868E12, -1.954399520371328E12, -0.229898414537091E12,
      1.230889276648774E3, 4.190283177829139E3, 0.092524698681849E3, Some(sunFrame))
  ...
  it should "convert the Cartesian elements from/to Keplerian for the Earth" in {
    val k_true = new Keplerian(149.598261E9, 0.01671123, 0.124878, 1.99330267, 6.08665, 0, Some(sunFrame))

    val pv = PosVel(-0.331617962895771E11, 1.423339778393807E11, 0.167105043530553E11,
      -2.949775938428787E4, -0.669095631311988E4, -0.154682704715132E4, Some(sunFrame))
  ...
  it should "convert the Cartesian elements from/to Keplerian for 2000 NL10" in {
    ...
    val pv = PosVel(1.568729987733919E11, 0.236439228464126E11, 0.761836182030944E11,
      -1.634472864086562E4, 1.073863544602837E4, -1.246760056449113E4, Some(sunFrame))
    val k_true = Keplerian(AU.convert(0.9143), 0.8171, Deg.convert(32.52),
      Deg.convert(281.56), Deg.convert(237.44), TrueAnomaly(3.57456), Some(sunFrame))
  ...
  it should "convert the Spherical elements from/to Keplerian for the Moon" in {
    ...
    val pv = PosVel(-1.15037391547548e+008, -3.64408052048568e+008, -1.21513857899935e+007,
      +9.67536480042629e+002, -3.46497896859408e+002, +8.78970130662041e+001, Some(jplEarthFrame))
    ...
    val s = new Spherical(3.823277207168130E+08, -1.876578287892178E00, -3.178799710706510E-2, 1.031461835283901E+03,
      math.Pi / 2.0 - 1.535552685601132E00, 1.484255164403220E00, Some(jplEarthFrame))
